Public Class FrmConProdutos
    Dim objpro As New ClsProduto

    Private Sub tsbCancelar_Click(sender As Object, e As EventArgs) Handles tsbCancelar.Click
        Me.Close()
    End Sub

    Private Sub FrmConProdutos_Activated(sender As Object, e As EventArgs) Handles Me.Activated
        Me.Top = 155
        Me.Left = 627
    End Sub

    Private Sub TxtCampo_TextChanged(sender As Object, e As EventArgs) Handles TxtCampo.TextChanged
        DgdGrade.DataSource = objpro.Localizar(TxtCampo.Text)
    End Sub

    Private Sub DgdGrade_CellContentDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles DgdGrade.CellContentDoubleClick
        If MdiParent Is FrmMenu Then
            FrmCadProduto.Show()
            FrmCadProduto.txtCod.Text = DgdGrade.CurrentRow.Cells(0).Value
            FrmCadProduto.txtNom.Text = DgdGrade.CurrentRow.Cells(1).Value
            FrmCadProduto.txtPre.Text = DgdGrade.CurrentRow.Cells(2).Value
            FrmCadProduto.txtAtu.Text = DgdGrade.CurrentRow.Cells(3).Value
            FrmCadProduto.txtMin.Text = DgdGrade.CurrentRow.Cells(4).Value
            FrmCadProduto.txtMax.Text = DgdGrade.CurrentRow.Cells(5).Value
        End If
    End Sub

    Private Sub DgdGrade_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DgdGrade.CellContentClick

    End Sub

    Private Sub BunifuImageButton1_Click(sender As Object, e As EventArgs) Handles BunifuImageButton1.Click
        DgdGrade.DataSource = objpro.viewProdutos()
    End Sub

    Private Sub FrmConProdutos_Load(sender As Object, e As EventArgs) Handles MyBase.Load

    End Sub
End Class